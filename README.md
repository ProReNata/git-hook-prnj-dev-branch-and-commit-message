...
1. Enforce a branch naming standard that all branches should be named
   `[hotfix-]PREFIX-<number>-DEV-<number>-<description>`
2. Add the `PREFIX-<number>` and the `DEV-<number>` to the end of the commit message, or
   optionally just verify that they are there, based on an environment flag
...
: Yes, export set an environment flag `TICKET_BRANCH_COMMIT_MSG_AUTO_APPEND=0`
